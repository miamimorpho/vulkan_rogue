...
declare output default state
add listerners
process listerners to mutate output state
draw output state


LUA -> Draw Instructions -> C
C -> Input State -> Lua

Label some string, fg, bg
Label.on_hover
Label.on_click
localise input to a zone
...
Single Chars
Strings
Buttons register functions for (on hover) and (on click)
