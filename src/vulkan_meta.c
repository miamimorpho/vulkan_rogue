...
{
  /* estimated size is a measurement of how many characters are
   * like to be on screen at once and vertices/indices are
   * scaled proportionally by that number. Highest recorded yet: 10000
   */
  gfxBufferCreate(gfx.allocator,
		  VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
		  estimated_size * sizeof(vertex2) * 4,
		  dest_vertices);

  GfxBuffer* dest_indices = (GfxBuffer*)malloc(sizeof(GfxBuffer));
  gfxBufferCreate(gfx.allocator,
		  VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
		  estimated_size * sizeof(uint32_t) * 6,
		  dest_indices);
